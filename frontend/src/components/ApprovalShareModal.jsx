...
// Shown right after an approval link is created.
//
// Order matters here (2026-07-17): the Reviewtap System send is the PRIMARY
// action — comms belong in GHL on the contact record, not on a personal phone.
// wa.me is the fallback, and every share is logged so the order card can show
// the client was contacted. See LogoRequestShareModal for the same rationale.
export default function ApprovalShareModal({ result, clientName, onClose }) {
  ...
  async function copyLink() {
    ...
    logShare({ kind: 'approval', token: result.token, channel: 'copy' })
  }
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="card w-full max-w-md p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-gray-900">Approval link ready</h2>
            <p className="text-xs text-gray-400 mt-0.5">{clientName ? `For ${clientName} — ` : ''}the client taps Approve or Request changes. You'll see their answer on the order.</p>
          </div>
        ...
        <div className="space-y-1">
          <a href={result.waUrl} target="_blank" rel="noopener noreferrer"
            onClick={() => logShare({ kind: 'approval', token: result.token, channel: 'whatsapp' })}
            ...
            {canUseSystem ? 'Send from my own WhatsApp instead' : 'Send on WhatsApp'}
          </a>
          {canUseSystem && (
